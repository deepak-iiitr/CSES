...
public class Main {
    ...
    static boolean BFS(ArrayList<Integer>[] adj,int s,int[] colour,boolean[] visit){
        int n=adj.length;
        ...
        while(!queue.isEmpty()){
            int u=queue.remove();
            int c=(colour[u]==1)?2:1;
            for(int i=0;i<adj[u].size();i++){
                if(!visit[adj[u].get(i)]){
                    visit[adj[u].get(i)]=true;
                    queue.add(adj[u].get(i));
                    colour[adj[u].get(i)]=c;
                }else{
                    if(colour[adj[u].get(i)]==colour[u])
                        return false;
                }
            }
        }
