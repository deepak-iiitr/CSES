        ...
        while(t-->0) {
            ...
            for(int i = 0;i<n;i++){
                if(!hmap.containsKey(arr[i]))
                    hmap.put(arr[i],1);
                distinct[i] = hmap.size();
            }
            long count = 0;
            TreeMap<Integer,Queue<Integer>> tmap = new TreeMap<>();
            for(int i = 0; i<n;i++){
                Queue<Integer> q = new LinkedList<>();
                if(tmap.containsKey(distinct[i]))
                    q = tmap.get(distinct[i]);
                q.add(i);
                tmap.put(distinct[i],q);
            }
            for(int i = 0;i<n;i++){
                if(tmap.containsKey(distinct[i]+k-1)){
                    Queue<Integer> st = tmap.get(distinct[i]+k-1);
                    int index = st.remove();
                    long ele = index - i+1;
                    count += (ele*(ele-1))/2;
                    tmap.put(distinct[i]+k-1,st);
                    count++;
                }else{
                    long ele = n-i+1;
                    count += (ele*(ele-1))/2;
